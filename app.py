...
# Unit Conversion Mappings #integrated
UNIT_CONVERSIONS = {
    # Volume conversions
    'ml': {'ml': 1, 'l': 0.001, 'lt': 0.001, 'ltr': 0.001},
    'l': {'ml': 1000, 'l': 1, 'lt': 1, 'ltr': 1},
    'lt': {'ml': 1000, 'l': 1, 'lt': 1, 'ltr': 1},
    'ltr': {'ml': 1000, 'l': 1, 'lt': 1, 'ltr': 1},
    # Weight conversions
    'g': {'g': 1, 'kg': 0.001},
    'kg': {'g': 1000, 'kg': 1},
    # Piece conversions
    'pcs': {'pcs': 1, 'pc': 1, 'piece': 1, 'pieces': 1},
    'pc': {'pcs': 1, 'pc': 1, 'piece': 1, 'pieces': 1},
}

class UnitConverter: #integrated
    """
    Handles unit conversion and normalization for volume, weight, and quantity units.
    Supports conversions like 1.5L ↔ 1500ml, 2kg ↔ 2000g, etc.
    """
    
    @staticmethod
    def extract_quantity_and_unit(text: str) -> Optional[Tuple[float, str]]:
        """
        Extract quantity and unit from text.
        Examples: "1.5L" → (1.5, "L"), "1500ml" → (1500, "ml"), "2kg" → (2, "kg")
        """
    ...
    def normalize_unit(unit: str) -> Optional[str]:
        ...
        # Check aliases
        for standard_unit, conversions in UNIT_CONVERSIONS.items():
    ...
    def convert_quantity(quantity: float, from_unit: str, to_unit: str) -> Optional[float]:
        ...
        if from_unit_norm not in UNIT_CONVERSIONS or to_unit_norm not in UNIT_CONVERSIONS[from_unit_norm]:
            return None
        
        conversion_factor = UNIT_CONVERSIONS[from_unit_norm][to_unit_norm]
        return quantity * conversion_factor
    
    @staticmethod
    def are_quantities_equivalent(text1: str, text2: str, tolerance: float = 0.01) -> bool:
        """
        Check if two quantity+unit expressions represent the same value.
        Examples:
        - "1.5L" and "1500ml" → True
        - "2kg" and "2000g" → True
        - "1L" and "999ml" → False (within tolerance)
        """
        q1_unit = UnitConverter.extract_quantity_and_unit(text1)
        q2_unit = UnitConverter.extract_quantity_and_unit(text2)
        
        if not q1_unit or not q2_unit:
            return False
        ...
        # Normalize units
        u1_norm = UnitConverter.normalize_unit(u1)
        u2_norm = UnitConverter.normalize_unit(u2)
        
        if not u1_norm or not u2_norm:
            return False
        
        # Convert q1 to u2
        converted = UnitConverter.convert_quantity(q1, u1, u2)
        
        if converted is None:
            return False
        
        # Check if values are approximately equal (with tolerance)
        return abs(converted - q2) <= (q2 * tolerance)
    
    @staticmethod
    def normalize_text_with_units(text: str) -> str:
        """
        Normalize text by standardizing units to a canonical form.
        E.g., "Coke 1.5L" → "Coke 1500ml", "Sprite 2kg" → "Sprite 2000g"
        """
        def replace_unit(match):
            ...
            # Normalize volume units to ml
            if unit in ['l', 'lt', 'ltr']:
                converted_qty = quantity * 1000
                return f"{int(converted_qty) if converted_qty == int(converted_qty) else converted_qty}ml"
            
            # Normalize weight units to g
            elif unit == 'kg':
                converted_qty = quantity * 1000
                return f"{int(converted_qty) if converted_qty == int(converted_qty) else converted_qty}g"
            
            return match.group(0)
        
        pattern = r'(\d+\.?\d*)\s*([a-zA-Z]+)'
        return re.sub(pattern, replace_unit, text.lower())

# Fuzzy Matching Parameters #integrated
FUZZY_MATCH_CONFIG = {
    'threshold': 0.7,
    'top_n': 5,
    'prefix_weight': 1.0,
    ...
    'jaccard_weight': 0.8,
    'levenshtein_weight': 0.8,
    'unit_conversion_weight': 0.95  # New: weight for unit conversion matches
}


class AdvancedFuzzyMatcher: #integrated
    """
    Advanced Fuzzy Matching Engine with unit conversion support.
    """
    ...
    def get_ngrams(text: str, n: int) -> set:
        ...
        text = AdvancedFuzzyMatcher.normalize_text(text)
        return set(text[i:i+n] for i in range(len(text)-n+1))
    
    @staticmethod
    def jaccard_similarity(str1: str, str2: str, n: int = 2) -> float:
        """Calculate Jaccard similarity using n-grams"""
        ngrams1 = AdvancedFuzzyMatcher.get_ngrams(str1, n)
        ngrams2 = AdvancedFuzzyMatcher.get_ngrams(str2, n)
    ...
    def levenshtein_distance(s1: str, s2: str) -> int:
        ...
        s1_norm = AdvancedFuzzyMatcher.normalize_text(s1)
        s2_norm = AdvancedFuzzyMatcher.normalize_text(s2)
        ...
        previous_row = range(len(s2_norm) + 1)
    ...
    @staticmethod
    def prefix_match_score(query: str, target: str) -> float:
        """Check if query is a prefix or starts with target words"""
        query_norm = AdvancedFuzzyMatcher.normalize_text(query)
        target_norm = AdvancedFuzzyMatcher.normalize_text(target)
        ...
        for word in target_words:
            word_norm = AdvancedFuzzyMatcher.normalize_text(word)
    ...
    @staticmethod
    def substring_match_score(query: str, target: str) -> float:
    ...
    @staticmethod
    def unit_conversion_match_score(query: str, target: str) -> float:
        """
        Check if query and target represent equivalent quantities with different units.
        E.g., "1.5L" vs "1500ml", "2kg" vs "2000g"
        """
        # Extract quantities
        q1_unit = UnitConverter.extract_quantity_and_unit(query)
        q2_unit = UnitConverter.extract_quantity_and_unit(target)
        
        if not q1_unit or not q2_unit:
            return 0.0
        
        # Check if quantities are equivalent
        if UnitConverter.are_quantities_equivalent(query, target):
            # Check if the non-quantity parts match
            query_no_qty = re.sub(r'\d+\.?\d*\s*[a-zA-Z]+', '', query).strip()
            target_no_qty = re.sub(r'\d+\.?\d*\s*[a-zA-Z]+', '', target).strip()
            
            if query_no_qty.lower() == target_no_qty.lower():
                return 1.0
            else:
                # Partial match for product name
                similarity = SequenceMatcher(None, 
                    AdvancedFuzzyMatcher.normalize_text(query_no_qty),
                    AdvancedFuzzyMatcher.normalize_text(target_no_qty)
                ).ratio()
    ...
    def fuzzy_search(
        ...
        """
        Advanced fuzzy search combining multiple strategies including unit conversion.
        """
        if threshold is None:
            threshold = FUZZY_MATCH_CONFIG['threshold']
        if top_n is None:
            top_n = FUZZY_MATCH_CONFIG['top_n']
        ...
        for candidate in candidates:
            ...
            # Check unit conversion match first
            unit_score = AdvancedFuzzyMatcher.unit_conversion_match_score(query, candidate)
            if unit_score > 0:
                results.append({
                    'value': candidate,
                    'score': unit_score * FUZZY_MATCH_CONFIG['unit_conversion_weight'],
                    'method': 'unit_conversion'
                })
                continue
            
            # Standard fuzzy matching
            prefix_score = AdvancedFuzzyMatcher.prefix_match_score(query, candidate)
            substring_score = AdvancedFuzzyMatcher.substring_match_score(query, candidate)
            
            sequence_score = SequenceMatcher(None, 
                AdvancedFuzzyMatcher.normalize_text(query),
                AdvancedFuzzyMatcher.normalize_text(candidate)
            ).ratio()
            
            jaccard_score = AdvancedFuzzyMatcher.jaccard_similarity(query, candidate, 2)
            jaccard_score_3 = AdvancedFuzzyMatcher.jaccard_similarity(query, candidate, 3)
            
            lev_distance = AdvancedFuzzyMatcher.levenshtein_distance(query, candidate)
            max_len = max(len(AdvancedFuzzyMatcher.normalize_text(query)), 
                         len(AdvancedFuzzyMatcher.normalize_text(candidate)))
            lev_score = 1 - (lev_distance / max_len) if max_len > 0 else 0
            
            final_score = max(
                prefix_score * FUZZY_MATCH_CONFIG['prefix_weight'],
                substring_score * FUZZY_MATCH_CONFIG['substring_weight'],
                sequence_score * FUZZY_MATCH_CONFIG['sequence_weight'],
                jaccard_score * FUZZY_MATCH_CONFIG['jaccard_weight'],
                jaccard_score_3 * 0.75,
                lev_score * FUZZY_MATCH_CONFIG['levenshtein_weight']
            )
            ...
            method_scores = {
                ...
                'jaccard': max(jaccard_score, jaccard_score_3),
...
class FuzzySearch:
    """
    A robust system for fuzzy matching with support for:
    - Prefix matching
    - Spelling mistakes
    - Incomplete names
    - Exact match bypass
    """
    def __init__(self, products: List[str], customers: List[str],
        ...
        # FIX: Removed the local normalize_string function and now correctly
        # use the class method (self.normalize_string) to build the mappings.
        self.products_normalized = {self.normalize_string(p): p for p in self.products}
        self.customers_normalized = {self.normalize_string(c): c for c in self.customers}
 
        logger.info(f"FuzzySearch initialized with {len(products)} products and {len(customers)} customers.")
 
    def normalize_string(self, s: str) -> str:
        """Normalizes a string by lowercasing and replacing look-alike characters."""
        if not isinstance(s, str):
            return ""
        # Expanded normalization for better matching
        return s.lower().replace('×', 'x').replace('×', 'x').strip()
 
    def find_best_matches(self, query: str, entity_type: Literal["product", "customer"]) -> List[Tuple[str, float]]:
        """
        Find the best matching items using multiple matching strategies.
        """
        items_map = self.products_normalized if entity_type == "product" else self.customers_normalized
        query_normalized = self.normalize_string(query)
 
        # Case 2: Check for exact or near-exact match first
        exact_match = self._check_exact_match(query_normalized, items_map)
        if exact_match:
            logger.info(f"Exact match found for '{query}': '{exact_match}'")
            return [(exact_match, 1.0)]
 
        matches = []
        for normalized_item, original_item in items_map.items():
            final_score = self._calculate_weighted_score(query_normalized, normalized_item)
 
            if final_score >= self.threshold:
                matches.append((original_item, final_score))
 
        matches.sort(key=lambda x: x[1], reverse=True)
        return matches[:self.top_n]
 
    def _check_exact_match(self, query_normalized: str, items_map: Dict[str, str]) -> str:
        """Check if query exactly or almost exactly matches any item."""
        if query_normalized in items_map:
            return items_map[query_normalized]
 
        for norm_item, original_item in items_map.items():
            if fuzz.ratio(query_normalized, norm_item) > (self.exact_match_threshold * 100):
                return original_item
        return None
 
    def _calculate_weighted_score(self, query_norm: str, item_norm: str) -> float:
        """
        Calculate a final score from multiple fuzzy matching techniques to handle
        prefix (Case 1), typo (Case 3), and partial+typo cases (Case 4).
        """
        if query_norm == item_norm:
            return 1.0
 
        # Good for partial matches like "Sprite 330ml" in "Sprite 330ml 1x24..."
        partial_score = fuzz.partial_ratio(query_norm, item_norm) / 100.0
 
        # Robust against typos and extra words
        token_set_score = fuzz.token_set_ratio(query_norm, item_norm) / 100.0
 
        # Weighted score, giving higher weight to token_set for typo robustness
        score = (partial_score * 0.5) + (token_set_score * 0.5)
 
        # Cap the score to prevent it from being mistaken for a perfect 1.0 match
...
class AgentState(TypedDict):
    ...
    skip_fuzzy_search: bool
    fuzzy_search_strategy: Dict[str, str]
    like_suggestions: Dict[str, str]
...
class ContextAwareSynapseAgent:
    ...
    def __init__(self, azure_config: Dict):
        ...
        # Store for later use
        self.product_list = product_names #integrated line
        self.customer_list = customer_names #integrated line

        # Initialize Fuzzy Search
        # self.fuzzy_search_system = FuzzySearch(products=product_names, customers=customer_names)
        self.fuzzy_search_system = AdvancedFuzzyMatcher() #integrated line
...
""")
    ...
    def _create_graph(self) -> StateGraph: #integrated changes in this function
    ...
    def _extract_entities_for_fuzzy_search(self, user_question: str) -> Dict[str, List[str]]:
        ...
        RULES:
        1. **Products**: Look for beverage names, brands, sizes, packaging types, PARTIAL PRODUCT IDENTIFIERS
        - Examples: "Sprite Zero", "Coca Cola 330ml", "Fanta Orange Can"
        -  IMPORTANT: Include PARTIAL product mentions like "Coke 330", "Sprite 2L", "Fanta 250"
        - Include size/packaging if mentioned: "330ml", "Sleek Can", "4x6 NP"
        - Extract even if incomplete: "Coke 330" is valid (missing unit doesn't matter)
        ...
        3. **CRITICAL**: Extract PARTIAL product identifiers
        - "Coke 330" → Extract as "Coke 330" (fuzzy search will find "Coke 330ml")
        - "Sprite 2" → Extract as "Sprite 2" (fuzzy search will find "Sprite 2L")
        - "Fanta Orange" → Extract as "Fanta Orange"
        ...
        5. **BE AGGRESSIVE**: Extract anything that MIGHT be a product or customer name

        Return JSON with "products" and "customers" arrays.
        ...
        Query: "Show me customers who ordered Coke 330"
        Response: {{"products": ["Coke 330"], "customers": []}}

        Query: "List orders for Fanta 2L by ABC Company"
        Response: {{"products": ["Fanta 2L"], "customers": ["ABC Company"]}}
    ...
    def _fuzzy_search(self, state: AgentState) -> AgentState:
        """Perform fuzzy search and decide on a strategy: exact, like, or clarify."""
        #  CRITICAL FIX: Skip fuzzy search if already resolved via clarification
    #  CRITICAL: Check skip flag FIRST - before ANY processing
        if state.get("skip_fuzzy_search", False):
            logger.info("⏩ BYPASSING FUZZY SEARCH - Clarification already resolved")
            state["needs_clarification"] = False
            state["skip_fuzzy_search"] = False  # Reset for next query
            
            # Use corrected_question if set, otherwise user_question
            if not state.get("corrected_question"):
                state["corrected_question"] = state["user_question"]
            
            # Clear fuzzy-related state
            state["fuzzy_search_strategy"] = {}
            state["like_suggestions"] = {}
            
            logger.info(f" Using bypassed query: {state['corrected_question'][:100]}")
            return state
        user_question = state["user_question"]
        logger.info(f"Performing fuzzy search with unit conversion for: '{user_question}'")

        extracted_entities = self._extract_entities_for_fuzzy_search(user_question)
        product_entities = extracted_entities.get("products", [])
        customer_entities = extracted_entities.get("customers", [])

        # 🔧 NEW: Check if entities exist EXACTLY in database before fuzzy matching
        exact_matches_found = True
        for entity_type, entity in [("product", p) for p in product_entities] + [("customer", c) for c in customer_entities]:
            candidates = self.product_list if entity_type == "product" else self.customer_list
            
            # Check for exact match (case-insensitive)
            if not any(entity.lower() == candidate.lower() for candidate in candidates):
                exact_matches_found = False
                logger.info(f"❌ '{entity}' not found exactly in {entity_type} list - will run fuzzy search")
                break
            else:
                logger.info(f" '{entity}' found exactly in {entity_type} list - no fuzzy search needed")
        
        # 🔧 BYPASS FUZZY SEARCH if all entities exist exactly
        if exact_matches_found and (product_entities or customer_entities):
            logger.info(" All entities found exactly - bypassing fuzzy search")
            state["needs_clarification"] = False
            state["corrected_question"] = user_question  # Use original question as-is
            state["fuzzy_search_strategy"] = {}
            state["like_suggestions"] = {}
            logger.info(f"📝 Using original question: {user_question}")
            return state
        ...
        for entity_type, entity in all_entities:
            logger.info(f"🔎 Searching for {entity_type}: '{entity}'")
            
            candidates = self.product_list if entity_type == "product" else self.customer_list
            matches = AdvancedFuzzyMatcher.fuzzy_search(entity, candidates)

            if not matches:
                strategy[entity] = "exact"
                logger.warning(f"No fuzzy matches for '{entity}'. Using original term.")
                continue

            high_confidence_matches = [m for m in matches if m['score'] >= HIGH_CONFIDENCE_THRESHOLD]
            fuzz_suggestion_count = len(high_confidence_matches)

            top_5_matches_log = [(m['value'], f"{m['score']:.2f}", m['method']) for m in matches[:5]]
            logger.info(f"Fuzzy matches for '{entity}': {top_5_matches_log}")
            logger.info(f"📊 High confidence matches (>{HIGH_CONFIDENCE_THRESHOLD}): {fuzz_suggestion_count}")

            if fuzz_suggestion_count >= 3:
                strategy[entity] = "clarify"
                clarifications_needed[entity] = [m['value'] for m in high_confidence_matches[:5]]
                logger.info(f"Strategy for '{entity}': CLARIFY ({fuzz_suggestion_count} suggestions).")

            elif 1 <= fuzz_suggestion_count < 3:
                best_match_name = high_confidence_matches[0]['value']
                strategy[entity] = "like"
                like_suggestions[entity] = best_match_name
                logger.info(f"Strategy for '{entity}': LIKE (auto-correcting to '{best_match_name}')")
                corrected_question = re.sub(re.escape(entity), best_match_name, corrected_question, flags=re.IGNORECASE)
        ...
        #  CRITICAL FIX: Set needs_clarification flag properly
        if "clarify" in strategy.values():
            state["needs_clarification"] = True
            
            # Build clarification prompt
            prompt_parts = ["I found multiple possible matches for your query. Please help me clarify:\n"]
            ...
            # Store clarification context for later use
            ...
            #  CRITICAL: Log that we're stopping for clarification
            logger.info(f"🛑 STOPPING FOR CLARIFICATION - needs_clarification=True")
            logger.info(f"📋 Clarification options: {clarifications_needed}")
            
        else:
            state["needs_clarification"] = False
            logger.info(f" No clarification needed - proceeding with query")
        ...
        #  ADD: Final debug log
        logger.info(f"🏁 Fuzzy search completed - needs_clarification: {state['needs_clarification']}")
        logger.info(f"📝 Corrected question: {corrected_question}")
        
        return state

    def _validate_clarification_response(self, user_response: str, entity_suggestions: dict) -> bool:
        """
        Validate if user's clarification response is valid.
        Returns True if valid, False if user needs to be re-prompted.
        """
        user_lower = user_response.lower().strip()
        
        # Pattern 1: Contains any digit 1-5
        if any(str(i) in user_response for i in range(1, 6)):
            logger.info(f" Valid selection - contains digit")
            return True
        
        # Pattern 2: Contains number words
        number_words = ['one', 'two', 'three', 'four', 'five', 'first', 'second', 'third', 'fourth', 'fifth']
        if any(word in user_lower for word in number_words):
            logger.info(f" Valid selection - contains number word")
            return True
        
        # Pattern 3: Contains confirmation words with numbers
        if any(word in user_lower for word in ['yes', 'correct', 'right', 'that one', 'this one']):
            logger.info(f" Valid selection - confirmation word")
            return True
        
        # Pattern 4: Response matches one of the entity names
        for entity, options in entity_suggestions.items():
            for option in options:
                if option.lower() in user_lower or user_lower in option.lower():
                    logger.info(f" Valid selection - matches entity name '{option}'")
                    return True
        
        logger.warning(f"❌ Invalid selection: '{user_response}' - no recognizable pattern")
        return False
    ...
    **OUTPUT:**
    ...
    - NO markdown formatting (no **, no ```, no labels)
    - NO prefixes like "Corrected Query:" or "Output:"
    - NO quotation marks around the query
    - Just the plain corrected question
                                                                
    **EXAMPLE:**
    If original was: "Show sales for Sprite"
    And user selected option 2: "Sprite Zero 1.5L"
    Return: Show sales for Sprite Zero 1.5L
    **EXAMPLES:**
    ...
    """)
        ...
        try:
            ...
            #  REMOVE ANY LEADING MARKDOWN/LABELS
            corrected_query = re.sub(r'^\*\*CORRECTED QUERY:\*\*\s*["\']?', '', corrected_query).strip()
            corrected_query = corrected_query.strip('"')
    ...
    def _check_clarification_needed(self, state: AgentState) -> Literal["clarify", "continue"]:
        ...
        #  ADD: Debug logging
        logger.info(f"🔍 _check_clarification_needed called: needs_clarification={needs_clarification}")
        
        if needs_clarification:
            logger.info(f"🛑 Routing to CLARIFY")
            return "clarify"
        else:
            logger.info(f" Routing to CONTINUE")
            return "continue"
    ...
    def _handle_clarification(self, state: AgentState) -> AgentState:
        ...
        base_response = state.get("clarification_prompt", "I need clarification on your query.")
        ...
        #  CRITICAL: Clear these to stop SQL execution
        state["sql_query"] = ""
        state["query_result"] = pd.DataFrame()
        state["follow_up_questions"] = []
        state["chart_code"] = ""
        
        logger.info(f" Clarification response set: {state['final_response'][:100]}")
    ...
    **CRITICAL**: If query mentions "this year", "last year", "this quarter" with a specific customer/product, it is INDEPENDENT.
    ...
    def _extract_intent(self, state: AgentState) -> AgentState:
        ...
        Uses an LLM to distill the core user intent from a potentially conversational query.
            """
        #  CRITICAL: Use corrected_question if fuzzy search modified it
...
""")
    ...
    def _resolve_clarification(self, state: AgentState) -> AgentState:
        ...
        # 🔧 FIX: Check if user's response is valid (contains numbers or selection keywords)
        is_valid_selection = self._validate_clarification_response(user_response, pending_context["entity_suggestions"])
        
        if not is_valid_selection:
            logger.warning("⚠️ Invalid clarification response - re-prompting user")
            # Rebuild clarification prompt
            prompt_parts = ["I didn't understand your selection. Please choose by number:\n"]
            for entity, suggestions in pending_context["entity_suggestions"].items():
                prompt_parts.append(f"\n**Which '{entity}' did you mean?**")
                for i, suggestion in enumerate(suggestions, 1):
                    prompt_parts.append(f"  {i}. {suggestion}")
            ...
            # ❌ DO NOT clear pending_clarification - keep it for retry
            logger.info("🔁 Re-prompting user for valid clarification")
            return state
        ...
        #  EXTRA VALIDATION: Ensure we got back a valid product name
        suggestions = pending_context["entity_suggestions"]
        all_options = [opt for opts in suggestions.values() for opt in opts]
        
        #  ADD: Validate the corrected query
        if not corrected_query or len(corrected_query) < 5:
            logger.error("❌ Corrected query is too short or empty")
            state["error_message"] = "Failed to process clarification. Please try again."
            return state

        #  ADD: Extra cleaning pass
        corrected_query = corrected_query.replace("**CORRECTED QUERY:**", "").strip()
        corrected_query = corrected_query.strip('"\'')
        ...
        #  CRITICAL FIX: Add flag to skip fuzzy search on next pass
        state["skip_fuzzy_search"] = True
        logger.info("🚫 Set skip_fuzzy_search=True to prevent re-running fuzzy logic")
        ...
        logger.info("🎯 Pending clarification cleared - ready to proceed with query")
        return state

    def _generate_sql(self, state: AgentState) -> AgentState:
        """Generate SQL with intelligent context awareness for Azure Synapse."""
        #  CRITICAL: Use corrected_question if available (from fuzzy search)
        user_question = state.get("corrected_question") or state.get("user_intent") or state["user_question"]
        
        current_date = datetime.now().strftime('%Y-%m-%d')
        context = state.get("conversation_context", {})
        previous_queries = state.get("previous_queries", [])

        #  LOG what question is being used for SQL generation
        logger.info(f"Generating SQL for CORRECTED question: {user_question[:100]}...")
        if state.get("corrected_question"):
            logger.info(f"Original question was: {state['user_question'][:100]}")
            logger.info(f"Fuzzy search corrected it to: {state['corrected_question'][:100]}")
    ...
    **CRITICAL RULES:**
    ...
    6. **REMEMBER NEVER to USE LIKE Operator**, we will ask the user through fuzzy matching for the correct entity 
    **Database Schema:**
    {schema}
    ...
    **Tip:** Try rephrasing your query using one of the available values above. For example:
    ...
    def _generate_followup_sync(self, state: AgentState, context: Dict[str, Any]) -> List[str]:
        ...
        # Build visualization instruction based on whether user asked for chart
        viz_instruction = "Skip visualization question (user already asked for it)" if user_asked_for_chart else "First question should suggest a relevant chart"
        question_type_instruction = "All 3 should be data analysis questions." if user_asked_for_chart else "The first question should suggest a visualization."
        ...
        **RICH CONTEXT:**
        ```json
        {context_json}
        VISUALIZATION INSTRUCTION: {viz_instruction}
        RULES FOR CONTEXTUAL FOLLOW-UPS:
        Reference Specific Entities: Mention products, customers, or time periods from the current query
        Suggest Drill-Downs: If showing aggregated data, suggest viewing details
        Offer Comparisons: Compare with other time periods, categories, or segments
...
def sidebar_interface():
    ...
    with st.sidebar:
        logo_path = ".image/Coca-Cola_logo.svg.png" 
